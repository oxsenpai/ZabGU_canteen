...
public class Ordering_coffee_Activity extends AppCompatActivity{
    ...
    private void displayPrice(){
        DecimalFormat dFormatter = new DecimalFormat("р"+ "##,##0.00");
    ...
    private MenuItem displayHelper(int quantity){
        ...
        int size=SMALL_SIZE;
        if(sizeSpinner.getSelectedItem().toString().equals("Средний")) size=TALL_SIZE;
        if(sizeSpinner.getSelectedItem().toString().equals("Большой")) size=VENTI_SIZE;
        if(sizeSpinner.getSelectedItem().toString().equals("Грант")) size=GRANDE_SIZE;
