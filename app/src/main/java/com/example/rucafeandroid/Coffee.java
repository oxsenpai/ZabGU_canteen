...
public class Coffee extends MenuItem implements Customizable {
    ...
    public static final double SHORT_PRICE = 30;
    public static final double SIZE_PRICE_INCREASE = 10;
    public static final double ADD_ON_PRICE = 5;
    ...
    public String toString() {
        String output = "";
        if (size == SHORT_SIZE) output += "Маленький кофе ";
        if (size == TALL_SIZE) output += "Средний кофе ";
        if (size == VENTI_SIZE) output += "Большой кофе ";
        if (size == GRANDE_SIZE) output += "Грант кофе ";

        if (!cream && !syrup && !milk && !caramel && !whippedCream) return output + super.toString();

        output += ": с добавками: ";
        if (cream) output += "сливки, ";
        if (syrup) output += "сироп, ";
        if (milk) output += "молоко, ";
        if (caramel) output += "карамель, ";
        if (whippedCream) output += "взбитые сливки, ";
